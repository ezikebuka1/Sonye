...
const SKILL_CHIP_SELECTED: Record<SkillLevel, string> = {
  beginner:          'bg-[#D8EFDF] text-[#246B42] ring-2 ring-[#246B42]',
  advanced_beginner: 'bg-[#DCEBFF] text-[#15457B] ring-2 ring-[#15457B]',
  intermediate:      'bg-[#FFF1CC] text-[#8A5A00] ring-2 ring-[#8A5A00]',
  advanced:          'bg-[#D7E0EC] text-[#14304D] ring-2 ring-[#14304D]',
};
const CHIP_UNSELECTED = 'bg-white border border-[#CFE0F4] text-[#14304D]';
const GENDER_SELECTED = 'bg-[#EE5E00]/10 text-[#EE5E00] ring-2 ring-[#EE5E00]';
...
export function OnboardingForm({ flow, slotId, slotPreview }: Props) {
  ...
  return (
    <main className="min-h-screen bg-[#E6F0FF] pb-40">
      <div className="max-w-[390px] mx-auto px-5 pt-8">

        {/* Pinned banner — slot context only */}
        {bannerTxt && (
          <div className="mb-6 rounded-2xl bg-white border border-[#CFE0F4] px-4 py-3">
            <p className="text-xs font-medium text-[#5E80A3] uppercase tracking-wide mb-0.5">
              You&apos;re joining
            </p>
            <p className="text-sm font-semibold text-[#14304D]">{bannerTxt}</p>
            {fillShown && (
              <p className="text-xs text-[#5E80A3] mt-0.5">{slotPreview!.fill_count} already in</p>
            )}
          </div>
        )}

        {/* Header — locked copy per flow */}
        <h1 className="text-xl font-bold text-[#14304D] mb-1">
          {isSlot
            ? "phone verified. you're one step from the game."
            : "phone verified. let's find your game."}
        </h1>
        <p className="text-sm text-[#5E80A3] mb-7">Takes about a minute.</p>
        ...
        <form action={formAction} noValidate className="flex flex-col gap-6">
          ...
          {/* First name */}
          <div>
            <label htmlFor="first-name" className="block text-sm font-semibold text-[#14304D] mb-2">
              First name
            </label>
            <input
              id="first-name"
              ...
              placeholder="Jordan"
              className="w-full border border-[#CFE0F4] rounded-xl px-4 py-3 text-[#14304D] bg-white text-sm outline-none focus:border-[#8DBCF1] focus:ring-1 focus:ring-[#8DBCF1]"
            />
            {errors?.first_name && (
          ...
          {/* Last name */}
          <div>
            <label htmlFor="last-name" className="block text-sm font-semibold text-[#14304D] mb-2">
              Last name <span className="font-normal text-[#5E80A3]">(optional)</span>
            </label>
            <input
              id="last-name"
              name="last_name"
          ...
          <div>
            <span id="skill-label" className="block text-sm font-semibold text-[#14304D] mb-2">
              Skill level
            </span>
            <div
              role="radiogroup"
          ...
          <div>
            <span id="gender-label" className="block text-sm font-semibold text-[#14304D] mb-0.5">
              Gender <span className="font-normal text-[#5E80A3]">(optional)</span>
            </span>
            <p className="text-xs text-[#5E80A3] mb-2">
          ...
          {/* Sticky CTA */}
          <div className="fixed bottom-0 inset-x-0 bg-[#E6F0FF] pt-3 px-5 pb-[max(1.5rem,env(safe-area-inset-bottom))] max-w-[390px] mx-auto">
            <button
              type="submit"
              disabled={isPending}
              className="w-full bg-[#EE5E00] hover:brightness-95 disabled:opacity-60 text-white font-semibold rounded-xl py-3.5 transition-colors min-h-[52px]"
            >
